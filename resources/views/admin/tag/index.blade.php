                ...
                <div class="col-8 mt-3">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title ml-1">Количество: {{ $tagsCount }}</h3>
                        </div>
                        <div class="card-body">
                            ...
                            <x-table>
                                <x-slot name="thead">
                                    <x-table.tr>
                                        ...
                                        <x-table.th>Создан</x-table.th>
                                        <x-table.th>Действия</x-table.th>
                                    </x-table.tr>
                                </x-slot>
                                @foreach($tags as $tag)
                                    <x-table.tr>
                                        <x-table.td>{{ $tag->id }}</x-table.td>
                                        <x-table.td class="text-left">{{ $tag->title }}</a></x-table.td>
                                        <x-table.td>{{ $tag->posts->count() }}</x-table.td>
                                        <x-table.td>{{ $tag->created_at->translatedFormat('F d, Y') }}</x-table.td>
                                        <x-table.td class="pt-1">
                                            <x-form action="{{ route('admin.tags.destroy', $tag) }}" method="post">
                                                @method('DELETE')
